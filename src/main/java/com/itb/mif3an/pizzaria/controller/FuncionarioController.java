...
public class FuncionarioController {
    ...
    public ResponseEntity<Produto> listarProdutoPorId(@PathVariable(value = "id") String id) { //pathVariable parametro passado

        try {

            Long idLong = Long.parseLong(id);
            /*
            if (produtoService.findById(idLong) == null) {
                throw new NotFound("Produto não encontrado com o id " + id);
            }
            */
            return ResponseEntity.ok().body(produtoService.findById(idLong));

        }catch (NumberFormatException ex) {// Exception código será executado quando a excessao abaixo acontecer: {
            throw new BadRequest("'" + id + "'não é um número inteiro válido. Por favor, forneça um valor inteiro, como 42.");
        }
